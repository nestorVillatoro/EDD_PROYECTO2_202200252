module ModuloAVL
    use objetoImagen
    implicit none

    ! Cons
    integer, parameter :: LEFT_HEAVY = -1
    integer, parameter :: BALANCED = 0
    integer, parameter :: RIGHT_HEAVY = +1

    type Node_t
        
        type(objimagen) :: Value
        integer :: Factor
        type(Node_t), pointer :: Left => null()
        type(Node_t), pointer :: Right => null()

    end type Node_t

    type Tree_t
        type(Node_t), pointer :: root => null()
        contains
        procedure :: newTree
        procedure :: insert
        procedure :: generateGraph
        procedure :: BuscarID_Imagen
        procedure :: borrarID
        procedure :: existeID
    end type Tree_t

    contains

    function NewNode(value) result(nodePtr)
        type(Node_t), pointer :: nodePtr
        type(objimagen), intent(in) :: value
        allocate(nodePtr)
        nodePtr%Value = value
        nodePtr%Factor = 0
        nodePtr%Left => null()
        nodePtr%Right => null()
    end function NewNode

    subroutine newTree(self)
        class(Tree_t), intent(inout) :: self
        self%root => null()
    end subroutine newTree

    function rotationII(n, n1) result(result_node)
        type(Node_t), pointer :: n, n1, result_node
        
        n%Left => n1%Right
        n1%Right => n
        if (n1%Factor == -1) then
            n%Factor = 0
            n1%Factor = 0
        else
            n%Factor = -1
            n1%Factor = 1
        end if
        result_node => n1
    end function rotationII

    function rotationDD(n, n1) result(result_node)
        type(Node_t), pointer :: n, n1, result_node

        n%Right => n1%Left
        n1%Left => n
        if (n1%Factor == 1) then
            n%Factor = 0
            n1%Factor = 0
        else
            n%Factor = 1
            n1%Factor = -1
        end if
        result_node => n1
    end function rotationDD

    function rotationDI(n, n1) result(result_node)
        type(Node_t), pointer :: n, n1, result_node, n2
    
        n2 => n1%Left
        n%Right => n2%Left
        n2%Left => n
        n1%Left => n2%Right
        n2%Right => n1
        if (n2%Factor == 1) then
            n%Factor = -1
        else
            n%Factor = 0
        end if
        if (n2%Factor == -1) then
            n1%Factor = 1
        else
            n1%Factor = 0
        end if
        n2%Factor = 0
        result_node => n2
    end function rotationDI

    function rotationID(n, n1) result(result_node)
        type(Node_t), pointer :: n, n1, result_node, n2
        n2 => n1%Right
        n%Left => n2%Right
        n2%Right => n
        n1%Right => n2%Left
        n2%Left => n1
        if (n2%Factor == 1) then
            n1%Factor = -1
        else
            n1%Factor = 0
        end if
        if (n2%Factor == -1) then
            n%Factor = 1
        else
            n%Factor = 0
        end if
        n2%Factor = 0
        result_node => n2
    end function rotationID

    recursive function insert2(root, value, increase) result(result_node)
        type(Node_t), pointer :: root, result_node, n1
        logical, intent(out) :: increase
        type(objimagen), intent(in) :: value

        if (.not. associated(root)) then
            allocate(result_node)
            root => NewNode(value)
            increase = .true.
        else if (value%id_imagen < root%Value%id_imagen) then
            root%Left => insert2(root%Left, value, increase)
            if (increase) then
                select case (root%Factor)
                    case (RIGHT_HEAVY)
                        root%Factor = 0
                        increase = .false.
                    case (BALANCED)
                        root%Factor = -1
                    case (LEFT_HEAVY)
                        n1 => root%Left
                        if (n1%Factor == -1) then
                            root => rotationII(root, n1)
                        else
                            root => rotationID(root, n1)
                        end if
                        increase = .false.
                end select
            end if
        else if (value%id_imagen > root%Value%id_imagen) then
            root%Right => insert2(root%Right, value, increase)
            if (increase) then
                select case (root%Factor)
                case (RIGHT_HEAVY)
                    n1 => root%Right
                    if (n1%Factor == 1) then
                        root => rotationDD(root, n1)
                    else
                        root => rotationDI(root, n1)
                    end if
                    increase = .false.
                case (BALANCED)
                    root%Factor = 1
                case (LEFT_HEAVY)
                    root%Factor = 0
                    increase = .false.
                end select
            end if
        end if

        result_node => root
    end function insert2

    subroutine insert(tree, value)
        class(Tree_t), intent(inout) :: tree
        type(objimagen), intent(in) :: value
        logical :: increase
    
        increase = .false.
        tree%root => insert2(tree%root, value, increase)
    end subroutine insert

    subroutine GenerateGraph(this)
        class(Tree_t), intent(inout) :: this
        character(len=:), allocatable :: dotStructure
        character(len=:), allocatable :: createNodes
        character(len=:), allocatable :: linkNodes
        createNodes = ''
        linkNodes = ''
    
    
        dotStructure = "digraph G{" // new_line('a')
        dotStructure = dotStructure // "node [shape=circle];" // new_line('a')
    
        if (associated(this%root)) then
            call RoamTree(this%root, createNodes, linkNodes)
        end if
    
        dotStructure = dotStructure // trim(createNodes) // trim(linkNodes) // "}" // new_line('a')
        call write_dot(dotStructure)
        print *, "Archivo actualizado existosamente."
    end subroutine GenerateGraph

    recursive subroutine RoamTree(actual, createNodes, linkNodes)
        type(Node_t), pointer :: actual
        character(len=:), allocatable, intent(inout) :: createNodes
        character(len=:), allocatable, intent(inout) :: linkNodes
        character(len=20) :: address
        character(len=20) :: str_value
    
        if (associated(actual)) then
            ! SE OBTIENE INFORMACION DEL NODO ACTUAL
            address = get_address_memory(actual)
            write(str_value, '(I0)') actual%Value%id_imagen
            createNodes = createNodes // '"' // trim(address) // '"' // '[label="' // trim(str_value) // '"];' // new_line('a')
            ! VIAJAMOS A LA SUBRAMA IZQ
            if (associated(actual%Left)) then
            linkNodes = linkNodes // '"' // trim(address) // '"' // " -> "
            address = get_address_memory(actual%Left)
            linkNodes = linkNodes // '"' // trim(address) // '" ' &
                        // '[label = "L"];' // new_line('a')
    
            end if
            ! VIAJAMOS A LA SUBRAMA DER
            if (associated(actual%Right)) then
            address = get_address_memory(actual)
            linkNodes = linkNodes // '"' // trim(address) // '"' // " -> "
            address = get_address_memory(actual%Right)
            linkNodes = linkNodes // '"' // trim(address) // '" ' &
                        // '[label = "R"];' // new_line('a')
            end if
    
            call RoamTree(actual%Left, createNodes, linkNodes)
            call RoamTree(actual%Right, createNodes, linkNodes)
        end if
    end subroutine RoamTree

    function get_address_memory(node) result(address)
        !class(matrix_t), intent(in) :: self
        type(Node_t), pointer :: node
        character(len=20) :: address
        ! integer 8
        integer*8 :: i
    
        i = loc(node) ! get the address of x
        ! convert the address to string
        write(address, 10) i 
        10 format(I0)
    
    end function get_address_memory

    subroutine write_dot(code)
        character(len=*), intent(in) :: code
        open(10, file='graph/AVLgraph.dot', status='replace', action='write')
        write(10, '(A)') trim(code)
        close(10)
        ! Genera la imagen PNG
        call system("dot -Tpng graph/AVLgraph.dot -o graph/AVLgraph.png")

    end subroutine write_dot


    recursive function BuscarID_Imagen(self, id, capas_out) result(found)
        class(Tree_t), intent(in) :: self
        integer, intent(in) :: id
        integer, dimension(:), pointer :: capas_out
        logical :: found 
        
        found = .false.
        call buscarID(self%root, id, capas_out)
        
        if (size(capas_out) > 0) then
            found = .true.
        end if
    end function BuscarID_Imagen


    recursive subroutine buscarID(node, id_to_find, capas_out)
        type(Node_t), pointer :: node
        integer, intent(in) :: id_to_find
        
        integer, dimension(:), pointer :: capas_out

        !print *, "LLEGO EL ID A LA SEGUNDA",  id_to_find
        if (.not. associated(node)) then
            allocate(capas_out(0)) 
        else if (id_to_find == node%Value%id_imagen) then
            capas_out => node%Value%listaCapasImg
        else if (id_to_find < node%Value%id_imagen) then
            call buscarID(node%Left, id_to_find, capas_out)
        else
            call buscarID(node%Right, id_to_find, capas_out)
        end if
    end subroutine buscarID


    subroutine borrarID(self, id)
        class(Tree_t), intent(inout) :: self
        integer, intent(in) :: id
        logical :: decrease
    
        decrease = .false.
        call borrarNodo(self%root, id, decrease)
    end subroutine borrarID
    
    recursive subroutine borrarNodo(node, id_to_delete, decrease)
        type(Node_t), pointer :: node
        integer, intent(in) :: id_to_delete
        logical, intent(out) :: decrease
    
        type(Node_t), pointer :: temp, parent, successor
        integer :: balanceFactor
    
        if (.not. associated(node)) then
            decrease = .false.
        else if (id_to_delete < node%Value%id_imagen) then
            call borrarNodo(node%Left, id_to_delete, decrease)
            if (decrease) then
                select case (node%Factor)
                    case (RIGHT_HEAVY)
                        node%Factor = BALANCED
                    case (BALANCED)
                        node%Factor = LEFT_HEAVY
                        decrease = .false.
                    case (LEFT_HEAVY)
                        balanceFactor = node%Right%Factor
                        if (balanceFactor <= 0) then
                            node => rotationDD(node, node%Right)
                            if (balanceFactor == 0) then
                                node%Factor = RIGHT_HEAVY
                                node%Right%Factor = LEFT_HEAVY
                                decrease = .false.
                            else
                                node%Factor = BALANCED
                                node%Right%Factor = BALANCED
                            end if
                        else
                            node => rotationDI(node, node%Right)
                            node%Factor = BALANCED
                            node%Right%Factor = BALANCED
                            decrease = .false.
                        end if
                end select
            end if
        else if (id_to_delete > node%Value%id_imagen) then
            call borrarNodo(node%Right, id_to_delete, decrease)
            if (decrease) then
                select case (node%Factor)
                    case (LEFT_HEAVY)
                        node%Factor = BALANCED
                    case (BALANCED)
                        node%Factor = RIGHT_HEAVY
                        decrease = .false.
                    case (RIGHT_HEAVY)
                        balanceFactor = node%Left%Factor
                        if (balanceFactor >= 0) then
                            node => rotationII(node, node%Left)
                            if (balanceFactor == 0) then
                                node%Factor = LEFT_HEAVY
                                node%Left%Factor = RIGHT_HEAVY
                                decrease = .false.
                            else
                                node%Factor = BALANCED
                                node%Left%Factor = BALANCED
                            end if
                        else
                            node => rotationID(node, node%Left)
                            node%Factor = BALANCED
                            node%Left%Factor = BALANCED
                            decrease = .false.
                        end if
                end select
            end if
        else
            if (.not. associated(node%Left)) then
                temp => node
                node => node%Right
                deallocate(temp)
                decrease = .true.
            else if (.not. associated(node%Right)) then
                temp => node
                node => node%Left
                deallocate(temp)
                decrease = .true.
            else
                successor => node%Right
                do while (associated(successor%Left))
                    successor => successor%Left
                end do
                node%Value = successor%Value
                call borrarNodo(node%Right, successor%Value%id_imagen, decrease)
                if (decrease) then
                    select case (node%Factor)
                        case (LEFT_HEAVY)
                            node%Factor = BALANCED
                        case (BALANCED)
                            node%Factor = RIGHT_HEAVY
                            decrease = .false.
                        case (RIGHT_HEAVY)
                            balanceFactor = node%Left%Factor
                            if (balanceFactor >= 0) then
                                node => rotationII(node, node%Left)
                                if (balanceFactor == 0) then
                                    node%Factor = LEFT_HEAVY
                                    node%Left%Factor = RIGHT_HEAVY
                                    decrease = .false.
                                else
                                    node%Factor = BALANCED
                                    node%Left%Factor = BALANCED
                                end if
                            else
                                node => rotationID(node, node%Left)
                                node%Factor = BALANCED
                                node%Left%Factor = BALANCED
                                decrease = .false.
                            end if
                    end select
                end if
            end if
        end if
    end subroutine borrarNodo

    function existeID(self, id) result(found)
        class(Tree_t), intent(in) :: self
        integer, intent(in) :: id
        logical :: found
        integer, dimension(:), pointer :: capas_out

        found = buscarID_Imagen(self, id, capas_out)
    end function existeID

end module ModuloAVL